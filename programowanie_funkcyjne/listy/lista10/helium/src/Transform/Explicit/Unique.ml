...
let rec tr_eff_coercion env c =
  ...
  ; data =
    match c.data with
    | ECId eff       -> ECId (Env.tr_type env eff)
    | ECLift l       -> ECLift(Env.tr_type env l)
    | ECSwap(l1, l2) -> ECSwap(Env.tr_type env l1, Env.tr_type env l2)
    | ECConsL(l, c)  -> ECConsL(Env.tr_type env l, tr_eff_coercion env c)
    | ECConsR(c, l)  -> ECConsR(tr_eff_coercion env c, Env.tr_type env l)
    | ECComp(c1, c2) -> ECComp(tr_eff_coercion env c1, tr_eff_coercion env c2)
  }
...
let rec tr_val_coercion env c =
  ...
  ; data =
    match c.data with
    | VCId -> VCId
    | VCArrow(c1, c2, c3) -> VCArrow(
        tr_val_coercion env c1,
...
let rec tr_expr env e =
  ...
  ; data =
    ...
    | ECoerce(vc, ec, e) ->
      ECoerce(tr_val_coercion env vc, tr_eff_coercion env ec, tr_expr env e)
    | EExtern(name, tp) ->
