...
let ins_query = insert

let upper_query t x = 
  (match upper t x with 
  | None -> "BRAK"
  | Some x -> string_of_int x) ^ "\n" 


let lower_query t x = 
  (match lower t x with 
  | None -> "BRAK"
  | Some x -> string_of_int x) ^ "\n"

let process_line () =
  let line = read_line () 
  in let ins = String.get line 0
  in let x = String.sub line 2 (String.length line - 2) |> int_of_string 
  in (ins, x) 

let rec solve t = function 
  | 0 -> () 
  | n -> 
      let (ins, x) = process_line () 
      in match ins with 
      | 'I' -> solve (ins_query t x) (n - 1)
      | 'D' -> let (nt, ans) = del_query t x in print_string ans; solve nt (n-1)
      | 'L' -> print_string (lower_query t x); solve t (n-1)
      | _ -> print_string (upper_query t x); solve t (n-1)
