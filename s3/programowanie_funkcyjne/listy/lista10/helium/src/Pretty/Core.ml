...
let rec pretty_coercion env c =
  match c.data with
  | CLift l -> Box.box
    [ Box.word "lift"
    ; Box.ws (Box.indent 2 (pretty_type env 0 l))
    ]
  | CSwap(l1, l2) -> Box.box
    [ Box.box
      [ Box.word "swap"
  ...
  | CCons(l, c) ->
    ...
    ; pretty_coercion env c
    ]
  | CComp(c1, c2) ->
    Box.paren (Box.box
    [ Box.suffix (pretty_coercion env c1) (Box.word ";")
    ; pretty_coercion env c2
    ])
